...
watcher.checkSlot = async (pin, free) => {
    ...
    if (availableDates) {
        ...
        console.info(`${moment().format('MMMM Do YYYY, h:mm:ss a')} - Vaccination available - ${pin} - ${msgs.join(" // ")}`)
    } else {
        console.info(`${moment().format('MMMM Do YYYY, h:mm:ss a')} - No solt available at this moment!`)
        // process.exit();
    }
    // process.exit();
}
